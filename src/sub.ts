...
/**
 * type1がtype2のサブタイプであるか（type2にtype1が代入可能）を判定します
 * @returns true: type1がtype2のサブタイプ
 */
export function subtype(type1: Type, type2: Type): boolean {
    switch (type2.tag) {
        case "Boolean":
        case "Number": {
            return type1.tag === type2.tag;
        ...
        case "Func": {
            ...
            for (let i = 0; i < type1.params.length; i++) {
                if (!subtype(type2.params[i].type, type1.params[i].type)) {
                    return false;
                }
            }

            // 返り値: 共変(covariant)
            if (!subtype(type1.retType, type2.retType)) {
                return false;
            }
        ...
        case "Object": {
            ...
            for (const prop2 of type2.props) {
                const prop1 = type1.props.find((prop1) => prop1.name === prop2.name);
                if (!prop1) {
                    return false;
                }
        ...
        default: {
            throw new Error(`Not supported type: ${type2 satisfies never}`);
        }
    }
}
...
export function typecheck(t: Term, typeEnv: TypeEnv): Type {
    ...
    */
    switch (t.tag) {
        ...
        case "objectGet": {
            const objType = typecheck(t.obj, typeEnv);
